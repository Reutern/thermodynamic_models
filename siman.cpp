...
static inline void
copy_state(void *src, void *dst, size_t size)
{
    memcpy(dst, src, size);
}
...
void
gsl_siman_solve (const gsl_rng * r, gsl_vector* x0_p, void* predictor, gsl_siman_Efunc_t Ef,
...
size is given */
  ...
  x = (gsl_vector *) malloc (element_size);
  memcpy (x, x0_p, element_size);
  new_x = (gsl_vector *) malloc (element_size);
  best_x = (gsl_vector *) malloc (element_size);
  ...
  while (1) {
    ...
    for (i = 0; i < params.iters_fixed_T; ++i) {
      ...
      copy_state(x, new_x, element_size);

      take_step (r, new_x, params.step_size);
      new_E = Ef (new_x, predictor);
...
if (new_E < best_E) {
copy_state(new_x, best_x, element_size);
best_E = new_E;
}

        /* yay! take a step */
copy_state(new_x, x, element_size);
        E = new_E;
        ++n_eless;
                      ...
                      gsl_siman_params_t params)
{
  ...
  x = (gsl_vector*) malloc (params.n_tries * element_size);
  new_x = (gsl_vector*) malloc (params.n_tries * element_size);
  ...
  while (1)
    {
      ...
      for (i = 0; i < params.n_tries - 1; ++i)
        { /* only go to N_TRIES-2 */
          ...
          sum_probs[i] = 0;
          memcpy ((gsl_vector *)new_x + i * element_size, x, element_size);
          take_step (r, (gsl_vector *)new_x + i * element_size, params.step_size);
          energies[i] = Ef ((gsl_vector *)new_x + i * element_size, predictor);
          probs[i] = boltzmann(Ex, energies[i], T, &params);
        }
      /* now add in the old value of "x", so it is a contendor */
      memcpy ((gsl_vector *)new_x + (params.n_tries - 1) * element_size, x, element_size);
      ...
      for (i = 0; i < params.n_tries; ++i)
        {
          if (u < sum_probs[i])
            {
              memcpy (x, (gsl_vector *) new_x + i * element_size, element_size);
              break;
            }
        }
      if (print_position)

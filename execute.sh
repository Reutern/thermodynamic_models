...
-fo $OUTPUT_PATH/obs_pre_untrained_test.txt \
-pp $OUTPUT_PATH/par_untrained_test.par \
...
for i in `seq 1 0`
do
 echo "${yellow} untrained Run $i ${NC}"
 echo " "
./seq2expr \
-s $INPUT_PATH/seqs.fa  \
...
-p param.save \
-fo $OUTPUT_PATH/obs_pre_untrained_06.txt \
-pp $OUTPUT_PATH/par_untrained_06.par \
-i $INPUT_PATH/factor_info.txt \
-r $INPUT_PATH/rep.txt \
-o Direct	# modelOption: Logistic, Direct, Quenching, ChrMod_Unlimited, ChrMod_Limite
done
...
# -s ../data/Input/seqs_segal.fa  -e ../data/Input/expr_segal.tab -m ../data/Input/factors_untrained_eq.wtmx -c ../data/Input/coop.txt -f ../data/Input/factor_expr_segal.tab -fo ../data/obs_pre.txt -i ../data/Input/factor_info.txt -r ../data/Input/rep.txt -o Direct
 #-ff $INPUT_PATH/free_fix_indicator_file.txt -p testparam.save \-p testparam.save \ valgrind --leak-check=yes -p param.save \-p param.save \ -p $OUTPUT_PATH/par_untrained_06.par \
